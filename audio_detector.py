...
def extract_acoustic_features(audio_path: str) -> dict:
    try:
        ...
        print(
            f"Acoustic: mfcc={mfcc_std:.2f} zcr={zcr_std:.4f} "
            f"rms={rms_std:.4f} pitch={pitch_var:.2f} hints={hints}"
        )
        return {
            "acoustic_ai_score": hints / 4.0,
...
def detect_audio(audio_bytes: bytes, filename: str = "audio.wav") -> dict:
    ...
    try:
        ...
        print(
            f"Audio result: model={fake_score:.2f} acoustic={acoustic:.2f} "
            f"final={final:.2f} → {'FAKE' if is_fake else 'REAL'}"
        )
